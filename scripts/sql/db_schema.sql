...
create table VALUE (
  ...
  VALUE_LONG bigint,
  VALUE_DOUBLE real,
  VALUE_BIGDECIMAL decimal,
  VALUE_TYPE varchar(15) not null,
  primary key (ID)
);
ALTER TABLE VALUE ADD CONSTRAINT "C_VALUE_VALUE_TYPE" CHECK (VALUE_TYPE IN ('DIMENSION_VALUE', 'TIMESTAMP', 'STRING', 'INT', 'LONG', 'BIG_DECIMAL', 'DOUBLE'));
ALTER TABLE VALUE ADD CONSTRAINT "F_VALUE_FACT_VALUE_ID" FOREIGN KEY(FACT_VALUE_ID) REFERENCES FACT_VALUE(ID);
ALTER TABLE VALUE ADD CONSTRAINT "F_VALUE_DIMENSION_VALUE_ID" FOREIGN KEY(DIMENSION_VALUE_ID) REFERENCES DIMENSION_VALUE(ID);
